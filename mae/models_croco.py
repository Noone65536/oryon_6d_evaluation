...
class MAERobotLangCroco(nn.Module):
    ...
    def generate_token(self, text_processor, lang, device, max_length):
    ...
    def get_lang_embed_kws(self, processed_lang):
        lang_emb = self.clip_text(input_ids=processed_lang, return_dict=False)
        return lang_emb
    ...
    def get_representations_full_mask(
        self, imgs: torch.Tensor, language: Optional[Union[List[str], Tuple[str]]] = None, with_lang_tokens = True,
    ) -> torch.Tensor:
        """
        Use full mask token to objetin representations
        """
        device = imgs.device
        lang = self.generate_token(self.tokenizer, language, device , 77)
        
        # Tile Language & Language Mask if mismatch with # images!
        #if not len(lang) == len(imgs):
        #    print(f"Language & Image batch sizes mismatched! Tiling Language & Language Mask to match Image batch size.")
        #    lang = repeat(lang, "b seq -> (bsz b) seq", bsz=imgs.size(0))
        #    lang_mask = repeat(lang_mask, "b seq -> (bsz b) seq", bsz=imgs.size(0))
        imgs = imgs[:,0,...]
     
        latent1, mask1, ids_restore1 = self.forward_encoder(imgs, mask_ratio=0.0)
        latent2, mask2, ids_restore2 = self.forward_encoder(imgs, mask_ratio=0.95)
        ...
        pred = self.prediction_head(out)
        pred = self.unpatchify(pred)
        pred = pred[0]
        pred = (pred - pred.min()) / (pred.max() - pred.min())
        pred = F.to_pil_image(pred)
        pred.show()

        return out
    

    def get_representations_encoder_only(
        self, imgs: torch.Tensor, language: Optional[Union[List[str], Tuple[str]]] = None, with_lang_tokens = True,
    ) -> torch.Tensor:
        """
        Use full mask token to objetin representations
        ...
        latent1, mask1, ids_restore1 = self.forward_encoder(imgs, mask_ratio=0.0)
        latent1 = self.decoder_embed(latent1)
        
        lang_emb = self.get_lang_embed(lang)[0]
     

        out = torch.cat([latent1, lang_emb], dim=1)

        return out

    
    def get_representations_cat_language(
        self, imgs: torch.Tensor, language: Optional[Union[List[str], Tuple[str]]] = None, with_lang_tokens = True,
    ) -> torch.Tensor:
        """
        Use full mask token to objetin representations
        ...
        out = torch.cat([out, lang_emb[0]], dim=1)
    ...
    def get_representations(
        ...
        return self.get_representations_encoder_only(imgs, language, with_lang_tokens)
    

    def forward_6d_full_mask(
        self, imgs: torch.Tensor, language: Optional[Union[List[str], Tuple[str]]] = None, with_lang_tokens = True,
    ) -> torch.Tensor:
        """
        Use full mask token to objetin representations
